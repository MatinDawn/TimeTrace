...
Page({
  ...
  async submitForAutofill() {
    ...
    if (isRemoteEnabled()) {
      ...
      this.submitHintTimer = setTimeout(() => {
        this.setData({
          submitHintVisible: true
        });
      }, 1200);
      try {
        const draft = await createRemoteDraftFromInput(text, this.data.showVoiceInput ? "voice" : "text");
        clearTimeout(this.submitHintTimer);
        this.submitHintTimer = null;
        this.setData({
          isSubmitting: false,
          submitHintVisible: false
        });
        ...
        logError("quickentry.submitForAutofill", error, {
          source: this.data.showVoiceInput ? "voice" : "text"
        });
        clearTimeout(this.submitHintTimer);
        this.submitHintTimer = null;
        this.setData({
